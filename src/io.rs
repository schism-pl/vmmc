...
fn draw_white_background(vmmc: &Vmmc, dt: &mut DrawTarget, scale: f64) {
    let x_dim = (vmmc.simbox().max_x().to_num::<f64>() * 2.0 * scale) as f32;
    let y_dim = (vmmc.simbox().max_y().to_num::<f64>() * 2.0 * scale) as f32;
...
fn render_particles(vmmc: &Vmmc, dt: &mut DrawTarget, scale: f64) {
    let x_off: f64 = vmmc.simbox().max_x().to_num::<f64>() * scale;
    let y_off: f64 = vmmc.simbox().max_y().to_num::<f64>() * scale;
    ...
    for p in vmmc.particles().iter() {
        let mut pb = PathBuilder::new();
        let x = (p.pos().x().to_num::<f64>() * scale + x_off) as f32;
        let y = (p.pos().y().to_num::<f64>() * scale + y_off) as f32;
        // pb.move_to(x,y);
        ...
        for (patch_idx, patch) in vmmc.simbox().morphology(p).patches().iter().enumerate() {
            ...
            let patch_x = (patch_center.x().to_num::<f64>() * scale + x_off) as f32;
            let patch_y = (patch_center.y().to_num::<f64>() * scale + y_off) as f32;
            pb.arc(
                patch_x,
                patch_y,
                (scale * patch.radius().to_num::<f64>()) as f32,
                0.0,
                2.0 * PI,
            );
            pb.close();
...
fn render_interactions(vmmc: &Vmmc, dt: &mut DrawTarget, scale: f64) {
    let x_off = vmmc.simbox().max_x().to_num::<f64>() * scale;
    let y_off = vmmc.simbox().max_y().to_num::<f64>() * scale;
    let source = rgba_solid(0, 0, 0, 0xff);
    ...
    for p0 in vmmc.particles().iter() {
        ...
        for &neighbor_id in interactions.iter() {
            ...
            let mut pb = PathBuilder::new();
            pb.move_to(
                (p0.pos().x().to_num::<f64>() * scale + x_off) as f32,
                (p0.pos().y().to_num::<f64>() * scale + y_off) as f32,
            );
            let pos1 = p0.pos() + sep;
            pb.line_to(
                (pos1.x().to_num::<f64>() * scale + x_off) as f32,
                (pos1.y().to_num::<f64>() * scale + y_off) as f32,
            );
            pb.close();
            let draw_path = pb.finish();
...
pub fn write_geometry_png(vmmc: &Vmmc, pathname: &str) {
    ...
    let scale = 100.0;
    let x_dim = vmmc.simbox().max_x().to_num::<f64>() * 2.0 * scale;
    let y_dim = vmmc.simbox().max_y().to_num::<f64>() * 2.0 * scale;
