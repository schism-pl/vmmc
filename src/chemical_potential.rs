...
fn maybe_remove_particle(vmmc: &mut Vmmc, chemical_potential: f64, rng: &mut Prng) {
...
fn maybe_insert_particle(vmmc: &mut Vmmc, chemical_potential: f64, rng: &mut Prng) {
    let num_particles = vmmc.particles().num_particles();
...
fn particle_exchange(vmmc: &mut Vmmc, chemical_potential: f64, rng: &mut Prng) {
    if rng.random::<bool>() {
...
pub fn maybe_particle_exchange(vmmc: &mut Vmmc, chemical_potential: f64, rng: &mut Prng) {
    let p_exchange = 1.0 / (1.0 + vmmc.particles().num_particles() as f64);
    if rng.random::<f64>() < p_exchange {
